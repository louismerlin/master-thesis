...
With the dis-cover tool \dam{I would add something like "with the discover tool we developped". Otherwise one does not know at this stage what dis-cover is.}, we are able to extract class information and the
...
Nevertheless, most reverse-engineering efforts have been focused towards C binaries, 
because analysis methods found this way will often work on C++ binaries too. \dam{And probably becasue it is easier, e.g. no execptions, etc.}
...
It enables the instrumentation \dam{Maybe explain in 1 sentence what instrumentation means and can be used for.} of projects when we do not have access to the 
source code.
...
mostly used as executable files, object code and shared libraries. \dam{Maybe mention that it is the most used executable file format on UNIX-based systems.}
...
name, size, offset and some flags). \dam{Maybe mention the difference between segments and sections.}
...
\dam{Most references are not correclty printed (at least for me not) - shown as ??. Just double check that references are correct when you print the PDF.}
...
This Animal is sure to overwrite the \textbf{speak} method and have whatever other 
...
\dam{Maybe add a static and dynamic cast in Figure 2.1 or in a separate figure.}
...
For an example, see the conditional dynamic cast \dam{See the conditional dynamic cast? You probably want to say something like: "Consider for example a conditional dynamic cast...", right?} between the two child classes
...
The RTTI itself is composed of a pointer to a VTable for the typeinfo class \dam{Where can we see this in the figure?}
...
is caught, binaries need some way of safely doing \textbf{stack unwinding}. \dam{Maybe add in one sentence what stack unwinding means?}
...
improve fuzzing capabilities. \dam{There are other applications than fuzzing, like e.g. retrofitting security measures. I would probably not only mention fuzzing.}
...
exceptions).
\dam{Maybe a sentence why it requires PIC to work would be interesting.}
...
\lou{TODO FIGURE : different small examples (multiple inheritance, diamond 
problem...)}

\ant{Agree with the todo. I think you should definitely explain RTTIs in 
detail here, how they fit into vtables etc. Take inspiration from the MARX 
paper. You can always remove detail, but adding it is harder!}
\ant{I would do something like you have done, but: show some diagrams. Here is a 
vtable, here is the multiple inheritence case, here is the multiple inheritence 
case with two/three classes, as you can see we only need the primary one. 
I would also potentially quote, or reword, parts of the cxx-abi here: what is in 
an RTTI entry and what do the key fields mean? In particular this part: https://itanium-cxx-abi.github.io/cxx-abi/abi.html#rtti-layout - you could put 
...
\ant{Examples, pictures and diagrams? I know they are quite time-consuming to 
do, but on the other hand they communicate the information quite effectively. 
Showing the tree structure (compilation unit->...) I think will be really 
helpful.}
\dam{Definitely agree!}
...
\ant{Once you have some diagrams of DWARF here, how to do this below will 
hopefully be quite obvious}
...
\dam{Maybe another figure could help on this as well.}
...
\ant{Maybe you can also talk about tradeoffs here. A heuristic-free way won't 
...
\lou{TODO FIGURE : hexdump of an example of vtable, rtti and string, with
...
\label{dwarfdump}
\end{figure}
\dam{For consistency, you could stick with the animal example.}
...
\dam{Maybe include the figure you had in the Cyber Alp Retreat presentation.}
...
\dam{One does not really understand why this is needed. OK, this is explained at the end of the next section. Maybe it would be easier if you would move the last paragraph of the next section here?}
...
\dam{I would probably mention that dis-cover can recover all of this samples.}
...
\dam{How did you finally check that all classes / inheritance was recovered?}
...
\dam{It would be interesting to know what percentage of effective classes was recovered.}
...
\dam{I would mention that you achieve 100%, but on less targets (when compared to MARX).}
...
\dam{Do these plugins exactly the same as dis-cover (without DWARF output)? Or have they less or more advanced features?}
...
binaries that can find polymorphic class hierarchies without using heuristics. \dam{but requiring RTTI (compared to MARX), right?}
