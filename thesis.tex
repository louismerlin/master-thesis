...
With dis-cover, the tool we developed, we extract class information and the
hierarchy tree from a stripped C++ binary, and write DWARF debug information
and symbols for the later use of that knowledge by any existing debugging
tool.
...
the capabilities of the RetroWrite static rewriting tool to support C++
binaries.
...
binaries need some way of safely doing \textbf{stack unwinding}.
...
In figure \autoref{excepttable} you can see a \textbf{.gcc\_except\_table} that 
...
Finally, we also studied the closed-source \textbf{zoom}~\cite{zoom} binary.
This test case is very interesting in two ways.
First, we found \textbf{6039 classes} in the binary, with \textbf{5601 edges}
...
\textbf{.eh\_frame} and \textbf{.gcc\_except\_table} sections of an ELF file.
