...
\section{Global constructors in C++}
\lou{Every 'translation unit' (cpp file, once all headers are pasted into it
by the compiler)  will add an entry to this list to initialize its globals.
It is important to making RW work for C++!}
...
In this project, we use very minimalistic DIE abbreviations, containing only
...
Using RetroWrite on binaries that had C++ exception handling would cause
crashes in the new binary, caused by the fact that the exceptions thrown were
never caught.
