...
support C++ binaries.
\lou{Also: the type information is available in RetroWrite for instrumentation}
...
\lou{Talk about the fact that speak has to be overwritten.}
...
\lou{Explain upcasting, downcasting, sidecasting. Walk through the cat
example.}
...
\lou{Mangling is also useful because certain characters need to be encoded,
like ::.}
...
This is made possible by computing relative positions instead of using
absolute addresses.
\lou{Also mention relocations as another way (for what cannot be made
relative).}
...
citation for ASLR ? => use one of the cited papers in the wiki article.}
...
\lou{Explain what static rewriting is in one sentence.}
...
\lou{These labels are what the assembler will use to compute the addresses of
functions and data offsets etc.}
...
\lou{I could talk about design info of DWARF stuff here (with details and
examples), to show off my DWARF skills}
...
\lou{Talk about except tables etc here}
...
the chrome browser and the nodejs runtime) at the cost of having a
probabilistic approach.

They use six heuristics to locate the virtual tables, related to the table
layout and content.

\lou{MARX could be a good candidate for future integration.}

\lou{CFIXX and HexType

=> First check how they get type information (maybe they could use dis-cover
for closed-source binaries).

=> Also check if they have cool class examples}


\lou{What else can we talk about in this chapter ?

=> The tools for gidra and IDA (too specific for these frameworks)

=> The hard-working DWARF}
