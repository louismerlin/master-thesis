...
This means transforming an address offset (+8) into a symbol (.LD1234) and
adding the appropriate label at that location.
...
\dam{@Antony: Could be a future improvement for RetroWrite, though not being so 
straightforward. Most use cases I currently have are actually stripped 
position-independent C binaries (and some compiled for ARMv7/32bit - could that 
theoretically be supported by RetroWrite?)}
...
\end{figure}
\dam{Maybe the "VTable for Animal" and "VTable for Dog" could have different colors.}
...
\dam{Maybe add a reference to the \emph{eu-unstrip} tool.}
...
\dam{Is it really the compiler re-adding the pointer, or rather the assembler?}
...
\dam{You could mention that these examples can be find in dis-cover's Github repo.}
