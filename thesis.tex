...
With \emph{dis-cover}, the tool we developed, we \emph{extract class
...
Through careful research of the way C++ exceptions are implemented, we augment
the capabilities of the \emph{RetroWrite} static rewriting tool to support C++
binaries.
...
By utilizing these two tools, we successfully extract useful information from
closed-source applications like Zoom, and instrument large and well-known
closed-source projects like {TODO}.
...
The dis-cover tool extracts information from a C++ binary, and re-inject it as
debug information using the DWARF format into the binary.
...
This is where Run-Time Type Information (\emph{RTTI}) comes into the picture.
...
(\emph{VTables}), which point to them.
...
a pointer to the virtual table (\emph{VTable}) for the Animal class.
...
containing a pointer to the \emph{frame\_dummy}. This method is in charge of
...
RetroWrite only works on \emph{x86\_64} position-independent executables
...
positions (pointers) in you binary are indistinguishable from data, and it
...
hoped to expose exception information to DWARF as well, but this would require
an extension to the DWARF format. However, by adding 
...
definitely was an awkward process, mostly because it was not displayed well in
...
By adding special treatment for that scenario, we resolved this failure.
...
was to address the \emph{init\_array} errors described in
\autoref{retrowritedesign}.
This meant treating the \emph{frame\_dummy} method differently, as it was
...
This table, contained in the \texttt{.gcc\_except\_table} section, has offsets
...
if the compiler was told to keep the Run-Time Type Information.
