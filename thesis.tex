...
int main (int argc, char *argv[]) {
  ...
  if (argc == 2) {
    ...
    // We can instanciate an instance of a subclass into a pointer of type
    // Animal*.
...
This means that the system needs to safely release memory that is outside of
the scope reached after an exception is caught.
See \autoref{unwinding} for an example of code that will only guarantee safety
if there is stack unwinding.
...
void this_method_might_throw(int x) {
  // This buffer is instanciated.
  char* this_object_could_leak = new char[1024];

  // If the throw is called, the buffer will stay instanciated if there is
  // no stack unwinding.
...
\end{lstlisting}
\caption{Example of C++ exception throwing and catching to illustrate stack unwinding}
\label{unwinding}
...
positions (pointers) in you binary are undistinguishable from data, and it
becomes an unsolvable problem to differenciate between data and a pointer
(which is why you would need heuristics in that scenario).

\lou{@antony could you please verify the formulation of this last paragraph ?}
