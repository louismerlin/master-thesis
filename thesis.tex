...
Once this was done, we could rewrite basic C++ binaries that used
exceptions.
...
Luckily, the \emph{eu-unstrip}~\cite{euunstrip} tool was created as an utility
application to "combine stripped files with separate symbols and debug
information".
...
We then let the assembler add a pointer to the \texttt{frame\_dummy} it just
created to the \texttt{init\_array} at assembly time.
...
problem).
These examples can be found in the code repository~\cite{discovergithub} for
the dis-cover project.
...
We tested the new C++ capabilities of RetroWrite on examples we had coded
ourselves.
