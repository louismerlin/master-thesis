...
of a class that inherit from Animal), and will contain pointers to the virtual
...
\dam{Great - I now like this section!}
...
\dam{Great again - Before your project I was never thinking about how exceptions are handled in binaries. I learned a lot... :)}
...
\dam{I would maybe go for TitleCase styling ("DWARF Debugging Standard"). But that is up to what you prefer more. Just be consistent (e.g. "Related Work").}
...
\dam{I would mention the term "reassembleable assembly" in the above section. Because that is what you describe and is the terminology used in the RetroWrite paper.}

RetroWrite only works on \emph{x86\_64} unstripped position-independent
executables (see \autoref{picpie}), compiled from C.
This enables RetroWrite to stay heuristics-free (meaning that it is
...
\dam{@Antony: Could be a future improvement for RetroWrite, though not being so straigh forward. Most use cases I currently have are actually stripped position-independent C binaries (and some compiled for ARMv7/32bit - could that theoretically be supported by RetroWrite?)}
